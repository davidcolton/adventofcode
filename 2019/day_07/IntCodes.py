class IntCodes:
    ...
    def __output__(self, index, modes):
        ...
        """
        validated, modes = self.__validate__(index, 1, modes)
        output_code = self.stack[validated[0]] if modes[0] == "0" else validated[0]
        print(output_code)
        self.output_codes.append(output_code)
        return index + 1
    ...
    def run_program(self, program, input=None):
        ...
        return exit_code, self.output_codes, self.stack
...
with open("./input.txt") as f:
    input_codes = f.read().strip()
    codes = [int(n) for n in input_codes.split(",")]
    print(codes)

exit_code = decoder.run_program(codes, 5)
print(f"my exit code: {exit_code[1][-1]}")
