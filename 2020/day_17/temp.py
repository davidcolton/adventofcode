...
def get_neighbour_coordinates(p):
    deltas = [
        x for x in product([-1, 0, 1], repeat=len(p)) if not all(i == 0 for i in x)
    ]
...
def get_coords_to_check(coord_data, neighbours):
    ...
    on_neighbours = [
        *reduce(op.add, [neighbours[c] for c, v in coord_data.items() if v == "#"], [])
    ]
...
def solve(inital_coordinates):
    ...
    while t < 6:
        ...
        for c in get_coords_to_check(coordinate_data, neighbours):
            ...
            local_count = sum(
                coordinate_data.get(x, ".") == "#" for x in get_neighbour_coordinates(c)
            )
